...
essential = df_tangsel_pool[df_tangsel_pool['nama_industri'].str.contains('L essential', case=False, na=False)]
unicare = df_tangsel_pool[df_tangsel_pool['nama_industri'].str.contains('UNICARE BEAUTY KOSMETINDO', case=False, na=False)]

must_have = pd.concat([essential, unicare])
...
final_perusahaan_raw = []
for _ in range(207): final_perusahaan_raw.append(random.choice(list_perusahaan_tangsel))
for _ in range(94): final_perusahaan_raw.append(random.choice(list_perusahaan_tangerang))
for _ in range(74): final_perusahaan_raw.append(random.choice(list_perusahaan_kab_tangerang))

random.shuffle(final_perusahaan_raw)

final_perusahaan_scrambled = [scramble_company_name(name) for name in final_perusahaan_raw]
...
sample_pria = safe_sample(df_pool[df_pool['jenis_kelamin'] == 'Pria'], 199)
sample_wanita = safe_sample(df_pool[df_pool['jenis_kelamin'] == 'Wanita'], 176)
...
df_final.to_excel(OUTPUT_RESPONDEN, index=False)
